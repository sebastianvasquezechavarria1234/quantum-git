  ...
  useFrame(() => {
    ...
    if (isAnimated) {
      ...
      if (glowRef.current.material && 'opacity' in glowRef.current.material) {
        (glowRef.current.material as THREE.Material).opacity = easeOut * 0.3
      }
    ...
    if (glowRef.current.material && 'opacity' in glowRef.current.material) {
      (glowRef.current.material as THREE.Material).opacity = 0.15 + Math.sin(elapsed * 1.5 + data.index) * 0.05
    }
  })
